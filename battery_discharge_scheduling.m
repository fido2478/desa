function bat_pack=battery_discharge_scheduling(i,interval,bat_pack,listofsch,...
        ...
        switch j
            case 1%kRR
                ...
                if 0
                    ...
                    ok=ceil(load_demand./(bat_pack(j).maxreC));
                    if ok(1)> num_cells
                        %k=ok(2);
                        k=ok(1)-ok(2); 
                    elseif ok(1)<1
                ...
                if dyn_thres > 0
                    ...
                    bat_pack(j).thres_g=bat_pack(j).turn*load_demand;%u_demand/max_cap * nomcells;% 0.2: 
                end
                %update GH
                ...
                if bat_pack(j).thres_g < bat_pack(j).shutdown
                    bat_pack(j).disconnect=0;
                    %listofsch=listofsch(find(listofsch ~= j));
                    bat_pack(j).otime_log=i;
                end;

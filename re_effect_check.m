function [bvolt ldisch]=re_effect_check(bat_pack,coeff_re)
    ...
    for i=1:length(ind)
        ...
        if bat_pack.ldisch(ind(i))>0
            if RT>1  
                ...
                polyval(coeff_re(RT,:),bat_pack.ldisch(i));
                %if re_volt > 0
                    bat_pack.bvolt(ind(i))=bat_pack.bvolt(ind(i))+abs(re_volt);
                %end

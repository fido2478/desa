c***********************************************************************
c     dualfoil.f  (version 5.1)  April 7, 2008
...
c
      ...
      npa=1 ! Number of particle sizes
...
c     Split of RG into its components. WHT
      RG=5.0d-3 !total resistance in foils, leads, and contacts, ohm-m2 WHT
      RG=0.0d0
...
c     If negative electrode is metal foil, let ep1=epp1=epf1=0.0
      ...
      read (1,*) rka1 !reaction rate constant for negative reaction
...
c      print *,'capacity ratio',(h1*ct1*(1-ep1-epp1-epf1-epg1))/
c     &(h3*ct3*(1-ep3-epp3-epf3-epg3))
c      print *,'Ah/m2, positive',(h3*cot3*(1-ep3-epp3-epf3-epg3)*rs3)
...
c     assume current density linear in electrodes
      ...
      vvold=OCP
      cur=1.0d0 ! guess current to get internal resistance
...
c Assumed temperature dependence for solid-state diffusion
      dfs1=dfs1save*dexp((EbarS1)*(t-298.0d0)/(t*298.0d0))
      dfs3=dfs3save*dexp((EbarS3)*(t-298.0d0)/(t*298.0d0))
...
c
      ...
      ranode=ranodesave*dexp((Ebarr1)*(298.0d0-t)/(t*298.0d0))
      rcathde=rcathdesave*dexp((Ebarr3)*(298.0d0-t)/(t*298.0d0))
...
c Use the individual pore-wall flux for film resistance. PSA
     ...
     &*(an1*ranode+an2*rcathde))
      ...
      b(2+mpa,1)=h2*de
      b(2+mpa,kp2)=h0*r1*pe
      b(2+mpa,kp1)=-b(2+mpa,kp2)
      b(2+mpa,2+mpa)=h1*de+h0*r1*g1*pe

c Modify Jacobians for individual pore-wall flux. PSA.

      if (kk.ge.2.and.mvdc1.eq.0.and.mvdc3.eq.0.and.izfl.eq.0) then
      b(2+mpa,2+mpa)=b(2+mpa,
     &2+mpa)+fc*b(2+mpa,kp2)*(an1*ranode+an2*rcathde)*pwfindiv_d(mpa,j)
      b(2+mpa,kj)=1.0d0
      else

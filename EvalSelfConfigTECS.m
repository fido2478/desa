...
fqov=10; %delta t= 5 or 10 in RTAS
vdegArt=0; dratio=0.995;
lifetime=50;
Cap=200; %100 fast vs 200 flow
...
if mod(gi,fqov)==0 | gi == 1
    ...
    if LCon.totV > DemandVolt & LCon.totV < StartVolt
        LCon.Vd=LCon.Vd;
    else
...
rl=size(power);
if Cap == 100
    plot([1:rl(2)],power(1,:),'--x','MarkerSize',10,'LineWidth',2);
    legend('2C');
else
    plot([1:rl(2)],power(1,:),':x','MarkerSize',10,'LineWidth',2);
    legend('C');
